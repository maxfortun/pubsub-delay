...
async function waitForKafka(admin: Admin, maxRetries = 30): Promise<void> {
...
async function waitForTopic(admin: Admin, topic: string, maxRetries = 30): Promise<void> {
  for (let i = 0; i < maxRetries; i++) {
...
async function waitForTopicDeleted(admin: Admin, topic: string, maxRetries = 60): Promise<void> {
  for (let i = 0; i < maxRetries; i++) {
    const topics = await admin.listTopics();
    if (!topics.includes(topic)) {
...
describe('PubSub Delay Integration Tests', { timeout: 120000 }, () => {
  ...
  it('should delay messages by the specified duration', async () => {
    ...
    assert.strictEqual(
      ...
      sentMessages.length,
      `Expected ${sentMessages.length} messages, got ${receivedMessages.length}`
    );

    // Verify timing of each message
    for (const sent of sentMessages) {
      const received = receivedMessages.find(
        (r) => r.headers['TEST_ID'] === sent.id
      );
      ...
      console.log(
        `Message ${sent.id}: expected ${expectedDelay}ms, actual ${actualDelay}ms, diff ${difference}ms`
      );

      assert.ok(
        difference <= toleranceMs,
        `Message ${sent.id} delay difference ${difference}ms exceeds tolerance ${toleranceMs}ms`
      );
    }
  });
  ...
  it('should create bucket topics dynamically', async () => {
    ...
    assert.ok(
      topics.includes(expectedBucketTopic),
      `Bucket topic ${expectedBucketTopic} should exist`
    );
  });
  ...
  it('should delete idle bucket topics after timeout', async () => {
    ...
    const topics = await admin.listTopics();
    assert.ok(
      !topics.includes(bucketTopic),
      `Bucket topic ${bucketTopic} should be deleted after idle timeout`
    );
  });
